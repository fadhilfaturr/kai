...
@section('jaringanm')
<button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#basicModal"><i class="ri-add-circle-line me-1"></i>Tambah Data Jaringan</button>

<form action="{{ route('jaringanm.store') }}" name="tambah" method="POST">
    @csrf
...
<table class="table table-bordered">
                            ...
                            <tbody>
                                @forelse ($jaringan_m as $index => $jaringan)
                                    <tr>
                                        <td>{{ $index + 1 }}</td>
                                        <td>{{ $jaringan->unit }}</td>
                                        ...
                                        <td class="text-center">
                                            <form onsubmit="return confirm('Apakah Anda Yakin ingin Menghapus Data?');"
                                                action="{{ route('jaringanm.destroy', $jaringan->id) }}" method="POST">
                                                <a href="{{ route('jaringanm.edit', $jaringan->id) }}"
