...
@section('hardwaret')
<button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#basicModal"><i class="bi bi-star me-1"></i>Tambah Data Hardware</button>

<form action="{{ route('hardwaret.store') }}" name="tambah" method="POST">
    @csrf
...
<table class="table table-bordered">
                            ...
                            <tbody>
                                @forelse ($hardware_t as $index => $hardware)
                                    <tr>
                                        <td>{{ $index + 1 }}</td>
                                        <td>{{ $hardware->unit }}</td>
                                        ...
                                        <td class="text-center">
                                            <form onsubmit="return confirm('Apakah Anda Yakin ?');"
                                                action="{{ route('hardwaret.destroy', $hardware->id) }}" method="POST">
                                                <a href="{{ route('hardwaret.edit', $hardware->id) }}"
